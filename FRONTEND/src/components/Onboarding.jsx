  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-end">
        <label className="text-zinc-500 font-bold uppercase text-xs tracking-widest">
          {label}
        </label>
            ...
            <div key={num} className="flex flex-col items-center flex-shrink-0 w-2">
              <div
                className={`w-0.5 rounded-full transition-all ${
                  isMajor ? "h-8 bg-white" : "h-4 bg-white/20"
                }`}
              />
              {isMajor && (
                <span className="text-[10px] mt-1 font-bold text-zinc-500">
                  {num}
                </span>
              )}
            </div>
...
export const Onboarding = () => {
  ...
  const steps = [
    ...
    },
    {
      title: "The Basics",
      description:
        "How are we built? This helps personalize your daily sugar shield.",
          ...
          <div className="grid grid-cols-3 gap-4">
            {["male", "female", "other"].map((g) => (
              <motion.button
                ...
                className={`p-4 rounded-2xl border-2 capitalize font-bold transition-all ${
                  profile.gender === g
                    ? "bg-blue-400/20 border-blue-400 text-blue-400 shadow-[0_0_20px_rgba(96,165,250,0.3)]"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950 p-4 overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <div className="min-h-[250px]">{steps[step].content}</div>
